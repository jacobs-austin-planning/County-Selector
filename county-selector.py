...
with st.sidebar:
    ...
    road_buffer_text = st.text_input("Buffer Road Distance (miles)")
    road_buffer = int(road_buffer_text) if road_buffer_text else None
...
def add_road_network_map():
    buffer_distance_roads = distance_conversion(road_buffer)
    buffered_line_road = roads_gdf['geometry'].buffer(buffer_distance_roads)
    buffered_line_road = gpd.GeoDataFrame(geometry=gpd.GeoSeries(buffered_line_road))
    intersecting_polygons_road = gpd.sjoin(counties_data, buffered_line_road, op="intersects").drop_duplicates(subset=['GEOID'])
    roads_buffer = folium.GeoJson(intersecting_polygons_road)
    st.session_state["markers"].append(roads_buffer)

    global map

    return intersecting_polygons_road
...
def make_linestring():
    ...
    if linestring and buffer_distance_miles:
        ...
        display_data['FIPS'] = display_data['FIPS'].astype(str)
...
def upload_csv(display_data):
    ...
    if uploaded_file is not None:
        ...
        merged_csv = display_data.merge(csv_data, how="inner", on="FIPS") #Add similar to or includes 'FIPS'
